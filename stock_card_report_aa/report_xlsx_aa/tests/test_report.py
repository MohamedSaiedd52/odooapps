...
class TestReport(common.TransactionCase):
    def setUp(self):
        ...
        self.xlsx_report = self.env["report.report_xlsx_aa.abstract"].with_context(
            active_model="res.partner"
        )
        self.report_name = "report_xlsx_aa.partner_xlsx"
    ...
    def test_currency_format(self):
        ...
        self.assertEqual(
            self.xlsx_report._report_xlsx_aa_currency_format(usd), "$#,##0.00"
        )
        eur = self.env.ref("base.EUR")
        self.assertEqual(
            self.xlsx_report._report_xlsx_aa_currency_format(eur), "#,##0.00 €"
        )
