...
class ShopifyPayout(models.Model):
    ...
    if hasattr(models, 'Constraint'):  # Odoo 19+ ignores _sql_constraints
        _unique_payout_reference = models.Constraint(
            'UNIQUE(payout_reference_id, instance_id)',
    ...
    def _cron_import_payouts(self):
        """Cron job to import payouts from all active instances"""
        ...
        for instance in instances:
            try:
                self.import_payouts_from_shopify(instance)
            except Exception as e:
                _logger.error("Cron payout import error for %s: %s", instance.name, str(e))
