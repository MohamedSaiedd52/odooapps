...
class ShopifyWorkflow(models.Model):
    ...
    if hasattr(models, 'Constraint'):  # Odoo 19+ ignores _sql_constraints
        _unique_workflow_status = models.Constraint(
            'UNIQUE(instance_id, shopify_status, workflow_type)',
    ...
    def _cron_process_workflows(self):
        ...
        # Find orders that need workflow processing
        ...
        for shopify_order in shopify_orders:
            try:
                if shopify_order.odoo_order_id:
                    # Find applicable workflows
                    workflows = self.search([
                        ('instance_id', '=', shopify_order.instance_id.id),
                        ('state', '=', 'active'),
                        ('auto_process', '=', True),
                    ])
                    for workflow in workflows:
                        try:
                            workflow.action_execute_workflow(shopify_order.odoo_order_id)
                        except Exception as e:
                            _logger.error("Workflow %s failed for order %s: %s",
                                        workflow.name, shopify_order.name, str(e))
            except Exception as e:
                _logger.error("Error processing workflows for order %s: %s",
                            shopify_order.name, str(e))
