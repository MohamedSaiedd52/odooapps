...
class ShopifyRisk(models.Model):
    ...
    def _send_email_notification(self):
        ...
        template = self.env.ref('ms_shopify_automation.email_template_risk_alert', raise_if_not_found=False)
        if template:
    ...
    def auto_detect_risks(self):
        ...
        for order in orders:
            try:
                # Find customer mapping if exists
                ...
                self.create({
                    'name': f'Risk Assessment for Order {order.shopify_order_number or order.shopify_order_id}',
                    'instance_id': order.instance_id.id,
                    'order_id': order.id,
                    'customer_id': customer_mapping.id if customer_mapping else False,
                    'risk_type': 'fraud',
                })
            except Exception as e:
                _logger.error("Error creating risk assessment for order %s: %s", order.name, str(e))
