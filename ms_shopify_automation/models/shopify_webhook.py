...
class ShopifyWebhook(models.Model):
    ...
    if hasattr(models, 'Constraint'):  # Odoo 19+ ignores _sql_constraints
        _unique_webhook_action_instance = models.Constraint(
            'UNIQUE(webhook_action, instance_id)',
    ...
    def _cron_health_check(self):
        """Cron job to check webhook health and reactivate failed webhooks"""
        webhooks = self.search([('state', 'in', ['active', 'error'])])
        for webhook in webhooks:
            try:
                # Check if webhook is still registered in Shopify
                instance = webhook.instance_id
                if instance.state != 'connected':
                    continue

                result = instance._shopify_request(f'webhooks/{webhook.webhook_id}.json')
                if not result or 'webhook' not in result:
                    # Webhook not found, try to recreate
                    webhook.action_create_webhook()
                    _logger.info('Webhook %s recreated successfully', webhook.name)
                else:
                    webhook.state = 'active'
            except Exception as e:
                webhook.state = 'error'
                _logger.error("Webhook health check error for %s: %s", webhook.name, str(e)) 