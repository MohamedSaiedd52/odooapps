...
def _arm_filter_action_views(action):
    ...
    if not hidden_types:
        return action
    # 'tree' and 'list' are used interchangeably by actions
    if 'tree' in hidden_types:
        hidden_types.add('list')
...
class ArmAction(Action):
    ...
    def load(self, action_id, additional_context=None):
        action = super().load(action_id, additional_context=additional_context)
...
class ArmExport(Export):
    ...
    def fields_get(self, model):
        ...
        if not _arm_available() or request.env.su or model in PROTECTED_MODELS:
            return fields
        hidden = request.env['access.rights.profile'].get_hidden_fields(model)
        for name in hidden:
            if name != 'id':
