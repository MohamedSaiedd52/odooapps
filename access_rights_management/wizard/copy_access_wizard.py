    ...
    def action_apply(self):
        ...
        if self.copy_groups:
            if self.mode == 'replace':
                targets.write({'groups_id': [(6, 0, self.source_user_id.groups_id.ids)]})
            else:
                targets.write({'groups_id': [(4, gid) for gid in self.source_user_id.groups_id.ids]})
        ...
        return {
            ...
            'params': {
                ...
                'message': _(
                    "The access of %(source)s was copied to %(count)s user(s).",
