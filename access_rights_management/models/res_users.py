...
class ResUsers(models.Model):
    ...
    def write(self, vals):
        # 19.0 renamed res.users.groups_id to group_ids
        tracked = 'access_profile_ids' in vals or 'group_ids' in vals
        before = {}
        if tracked and not self.env.context.get('arm_skip_change_log'):
            for user in self:
                before[user.id] = (user.group_ids, user.access_profile_ids)
        ...
        if tracked:
            ...
            for user in self:
                ...
                if 'group_ids' in vals:
                    Change._record_diff(user, 'group', old_groups, user.group_ids)
    ...
    def _arm_log_login(self, login, uid, status, ip):
    ...
    def _login(self, credential, user_agent_env):
        # 19.0: `_login` is a regular method (18.0 had a classmethod taking the
        # database name as its first argument). `credential` is a dict
        # ({'type', 'login', 'password'|...}) and the method returns an
        # `auth_info` dict, not a bare uid.
        ...
        try:
            ...
            self._arm_log_login(login, None, 'failed', ip)
            raise
        uid = auth_info['uid']
        ...
        if status:
            self._arm_log_login(login, uid, status, ip)
        ...
        if device_blocked:
            self._arm_log_login(login, uid, 'blocked_device', ip)
        ...
        self._arm_log_login(login, uid, login_status, ip)
        return auth_info
