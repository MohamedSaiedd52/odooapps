...
class AccessRightsSensitiveField(models.Model):
    ...
    _field_uniq = models.Constraint(
        'unique(field_id)',
        "This field is already flagged as sensitive.",
    )
    ...
    def _model_reader_users(self, model_name):
        ...
        users = accesses.mapped('group_id.all_user_ids').filtered(lambda u: not u.share)
        return users
    ...
    def _field_audience(self):
        ...
        if field.groups:
            readers = readers & field.groups.mapped('all_user_ids')
