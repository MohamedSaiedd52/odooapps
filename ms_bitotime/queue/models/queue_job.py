...
class QueueJob(models.Model):
    ...
    def init(self):
        # The queue_job_notify trigger is required by the jobrunner. It is
        # created here (and not only in the post_init_hook) so that it is also
        # installed when ms_bitotime is *upgraded* on an existing database.
        self._cr.execute(
            """
    ...
    def _message_post_on_failure(self):
        ...
        for record in self:
            ...
            if msg:
                record.message_post(body=msg, subtype_xmlid="ms_bitotime.mt_job_failed")

    def _subscribe_users_domain(self):
        """Subscribe all users having the 'Queue Job Manager' group"""
        group = self.env.ref("ms_bitotime.group_queue_job_manager")
        if not group:
            return None
        companies = self.mapped("company_id")
        # res.users.groups_id was renamed to group_ids / all_group_ids in Odoo 19
        domain = [("all_group_ids", "in", group.id)]
        if companies:
