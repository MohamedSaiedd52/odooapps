...
class ChannelManager:
    ...
    * get jobs to run
    ...
    def notify(
        ...
        if job:
            # db_name is invariant. The same uuid in two databases means one
            # db is a clone of the other; crashing the whole runner (upstream
            # asserts) would kill job processing for every db, so track the
            # first db only and warn about the duplicate.
            if job.db_name != db_name:
                _logger.warning(
                    "job %s found in db %s but already tracked in db %s "
                    "(cloned database?); ignoring the duplicate",
                    uuid, db_name, job.db_name,
                )
                return
