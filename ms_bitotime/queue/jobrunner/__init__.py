...
def _read_queue_job_config():
    misc = getattr(config, "misc", None)
    if misc is not None:  # Odoo <= 18
        return misc.get("queue_job", {})
    # Odoo 19: configmanager only parses [options]; read the [queue_job]
    # section straight from the rc file ourselves.
    import configparser
    ...
    try:
        ...
        if parser.has_section("queue_job"):
            return dict(parser.items("queue_job"))
...
try:
    ...
    queue_job_config = _read_queue_job_config()
