    ...
    def _has_queue_job(self):
        with closing(self.conn.cursor()) as cr:
            ...
            cr.execute(
                ...
                ("ms_bitotime", "installed"),
            )
            row = cr.fetchone()
            if not row:
                _logger.debug("ms_bitotime is not installed for db %s", self.db_name)
                return False
            # Multiple Odoo versions may share the same PostgreSQL cluster:
            # only handle databases whose ms_bitotime matches this server's
            # serie, so an 18.0 runner never executes jobs of a 19.0 db.
            if not (row[0] or "").startswith(odoo.release.serie):
                _logger.debug(
                    "db %s runs ms_bitotime %s which is not for serie %s, skipping",
                    self.db_name, row[0], odoo.release.serie,
                )
                return False
            cr.execute(
                """SELECT COUNT(1)
...
class QueueJobRunner:
    def __init__(
        ...
        self.db_by_name = {}
        self._stop = False
        # A socket pair instead of os.pipe(): on Windows, select() only
        # accepts sockets, so a pipe would crash wait_notification with
        # WinError 10038. socketpair() works on both platforms.
        self._stop_pipe = socket.socketpair()

    def __del__(self):
        # pylint: disable=except-pass
        try:
            self._stop_pipe[0].close()
        except OSError:
            pass
        try:
            self._stop_pipe[1].close()
        except OSError:
            pass
    ...
    def from_environ_or_config(cls):
        ...
        # http_interface is a *bind* address; "0.0.0.0"/"::" are not valid
        # *connect* addresses (Windows refuses them with WinError 10049)
        if host in ("0.0.0.0", "::"):
            host = "localhost"
        port = (
