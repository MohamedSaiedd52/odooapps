...
##############################################################################
#
#    Merged from rm_hr_attendance_sheet (Eng.Ramadan Khalil).
#
##############################################################################
...
class AttendanceSheetBatch(models.Model):
    ...
    category_ids = fields.Many2many("hr.employee.category",
                                    'att_sheet_batch_employee_category_rel',
                                    'att_sheet_batch_id', 'category_id',
                                    string="Tags")
    ...
    def gen_att_sheet(self):
        ...
        for batch in self:
            ...
            if batch.department_id and not batch.category_ids:
                domain.append(('department_id', '=', batch.department_id.id))
            elif not batch.department_id and batch.category_ids:
                domain.append(('category_ids', 'in', batch.category_ids.ids))
            elif batch.department_id and batch.category_ids:
                domain.append(('department_id', '=', batch.department_id.id))
                domain.append(('category_ids', 'in', batch.category_ids.ids))
