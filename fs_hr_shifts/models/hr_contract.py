...
##############################################################################
#
#    Merged from rm_hr_attendance_sheet (att_policy_id) and
#    fs_multi_shifts (multi-shift work entry source).
#
##############################################################################
...
class HrContract(models.Model):
    ...
    is_multi_shifts = fields.Boolean("Multi shifts")
    work_entry_source = fields.Selection(
        selection_add=[('calendar_planning', 'Working Schedule & Planning')],
        ondelete={'calendar_planning': 'set default'})

    def _get_attendance_intervals(self, start_dt, end_dt):
        res = super()._get_attendance_intervals(start_dt, end_dt)
        calendar_based_contracts = self.filtered(
            lambda c: c.work_entry_source == 'calendar_planning'
            and c.company_id.hr_attendance_overtime)
        if not calendar_based_contracts and res:
            return res

        if not calendar_based_contracts and not res:
            employees_by_calendar = defaultdict(
                lambda: self.env['hr.employee'])
            for contract in self:
                if contract.work_entry_source not in ['calendar_planning']:
                    continue
                employees_by_calendar[
                    contract.resource_calendar_id] |= contract.employee_id
            new_result = dict()
            for calendar, employees in employees_by_calendar.items():
                new_result.update(calendar._attendance_intervals_batch(
                    start_dt,
                    end_dt,
        ...
        attendances = self.env['hr.attendance'].sudo().search([
            ...
            ('check_in', '<=', end_dt),
            # We ignore attendances without check_out date
            ('check_out', '>=', start_dt),
        ])

        resource_ids = attendances.employee_id.resource_id.ids
        work_intervals_by_resources = {
            resource_id: WorkIntervals(list(intervals))
        ...
        for resource_id, intervals in res.items():
            ...
            for interval in intervals:
                ...
                work_intervals_by_resource_day[resource_id][day].append(
                    interval)

        lunch_intervals_by_resource = self._get_lunch_intervals(start_dt,
                                                                end_dt)
        ...
        for attendance in attendances:
            ...
            attendance_intervals = WorkIntervals(
                [(check_in_tz, check_out_tz, attendance)])

            contract = attendance.employee_id._get_contracts(
            ...
            if public_holiday:
                ...
                for (start, end, calendar_attendance) in work_intervals:
                    if start > holiday_end or end < holiday_start \
                            or check_in_tz > end or check_out_tz < start:
                        new_work_intervals.append(
                        ...
                        if start < check_in_tz:
                            new_work_intervals.append(
                                (start, check_in_tz, calendar_attendance))
                        if end > check_out_tz:
                            new_work_intervals.append(
                                (check_out_tz, end, calendar_attendance))
                work_intervals = WorkIntervals(new_work_intervals)
            lunch_intervals = lunch_intervals_by_resource.get(
                resource.id, WorkIntervals([]))
            ...
            if self.company_id.overtime_company_threshold:
                overtime_intervals = WorkIntervals([
                    ...
                    self.company_id.overtime_company_threshold])
            work_intervals_by_resources[resource.id] = \
                work_intervals | overtime_intervals
