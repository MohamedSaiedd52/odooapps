...
Response:
```json
{"status": "ok", "timestamp": "2026-02-09T10:34:06Z", "module_version": "17.0.1.0.0", "mcp_enabled": true}
```
...
> **Note:** `ODOO_DB` is **required for multi-database** servers. On Odoo 17, the bridge sends it as a `db` query parameter. On Odoo 19, it can also be read from the `X-Odoo-Database` header. For single-database setups it can be omitted.
