...
# Multi-db support:
# - Odoo 17: use `db` query parameter (sent per request below)
# - Odoo 19: can also read `X-Odoo-Database` header
if ODOO_DB:
...
def _api_get(path: str) -> dict:
    ...
    params = {"db": ODOO_DB} if ODOO_DB else None  # Odoo 17 db selection
    resp = client.get(f"/mcp/api/v1{path}", params=params)
...
def _api_post(path: str, data: dict) -> dict:
    ...
    params = {"db": ODOO_DB} if ODOO_DB else None  # Odoo 17 db selection
    resp = client.post(f"/mcp/api/v1{path}", json=data, params=params)
    resp.raise_for_status()
    return resp.json()
